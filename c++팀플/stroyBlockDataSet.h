...
public:
    Block1()
    {
        ...
        select1 = "무시하고 지나간다.";
        select2 = "대화에 끼어들어본다.";
        select3 = "시민을 도와 군인을 공격한다.";

        result1 = "괜히 갈등에 엮일 필요는 없다. 가던 길로 발걸음을 옮겼다.";
        result2 = "대화와 설득을 통해 시민의 딸을 되찾았다. 명성이 조금 올랐다.";
        result3 = "군인을 공격해 딸을 되찾을 수 있었다. 부상을 조금 입었으나 사례금을 받았다.";
...
public:
    Block3()
    {
        ...
        select1 = "장비를 강탈한다.";
        select2 = "약간의 돈과 함께 격려를 건넨다.";
        select3 = "그를 다독이며 응원해준다.";
...
public:
    Block4()
    {
        ...
        select1 = "조심스레 전리품을 훔쳐본다."; //선택지 1
        select2 = "지도에 위치를 기록하고 떠난다."; //선택지 2(선택)
        select3 = "도적떼를 소탕한다."; //선택지 3(선택)

        result1 = "무사히 잠입에 성공해 훔치는데 성공했다. 꽤 많은 돈을 얻을 수 있을 것 같다."; //돈+500, if 잠입력=<7(잠입실패, hp소폭 깎이고 도망침)
        result2 = "지도에 새로운 정보를 기록했다. 정보력이 늘었다."; //정보력 +100
        result3 = "예상보다 도적떼가 많았다. 상당한 피해를 입었다."; //hp -3
...
public:
    Block5()
    {
        ...
        select1 = "60일"; //선택지 1
        select2 = "59일"; //선택지 2(선택)
        select3 = "30일"; //선택지 3(선택)

        result1 = "틀렸네! 지혜를 좀 더 쌓으셔야겠구만! 이 말을 남기며 나그네는 사라졌다."; //선택지 1 결과
        result2 = "Block8로"; //선택지 2 결과(선택)
        result3 = "틀렸네! 지혜를 좀 더 쌓으셔야겠구만! 이 말을 남기며 나그네는 사라졌다."; //선택지 3 결과(선택)
...
public:
    Block6()
    {
        ...
        select1 = "131마리"; //선택지 1
        select2 = "31마리"; //선택지 2(선택)
        select3 = "1마리"; //선택지 3(선택)

        result1 = "틀렸네! 지혜를 좀 더 쌓으셔야겠구만! 이 말을 남기며 나그네는 사라졌다."; //선택지 1 결과
        result2 = "틀렸네! 지혜를 좀 더 쌓으셔야겠구만! 이 말을 남기며 나그네는 사라졌다."; //선택지 1 결과
        result3 = "Block8로"; //선택지 2 결과(선택)
...
public:
    Block8()
    {
        ...
        result1 = "나그네는 돈을 주고 떠났다."; //돈 +300
        result2 = "나그네는 빨간 물약을 건넸다. 물약을 마시니 상처가 낫고 강해진 느낌이 든다."; //hp+3,힘+2
        result3 = "나그네는 파란 물약을 건넸다. 물약을 마시니 머리 회전이 빨라진 느낌이 든다."; //카리스마+2,잠입능력+2
...
public:
    Block9()
    {
        ...
        select1 = "황급히 도망친다.";
        select2 = "무슨 일인지 뒤돌아 본다."; 
        select3 = "그들을 공격한다."; 

        result1 = "적국의 군인과 마주칠 이유는 없다. 황급히 자리를 떴다. 달리기가 빨라진 느낌이다."; //침투력 1+
        result2 = "그들은 돈을 주며 흘리지 말라고 해준다. 내 돈은 아니였지만 받고 무사히 넘어갔다."; //돈+100
        result3 = "공격하고 달아났다. 공격과정에서 약간의 부상을 입었으나 몸이 빨라진 느낌이다."; //hp-1, 침투력 1+
...
public:
    Block10()
    {
        ...
        select1 = "비싼 버섯인 것 같으니 챙긴다.";
        select2 = "그에게 버섯을 넘긴다."; 
        select3 = "버섯을 줄테니 버섯에 대해 알려달라고 한다."; 

        result1 = "그를 내쫓고 버섯을 집자마자 강한 고통이 느껴졌다. 아무래도 맹독버섯을 만진 것 같다."; //체력-4
        result2 = "낯선 사람은 돈을 주고 능숙하게 버섯을 채집해갔다."; //돈+
        result3 = "그는 그 버섯이 강한 독버섯임을 알려주고 다양한 정보를 알려줬다. 지식이 늘었다."; //정보력 1+
...
public:
    Block11()
    {
        ...
        select1 = "건달을 공격해 물건을 되찾아 준다.";
        select2 = "물건을 뺏아 상인에게 넘긴다."; 
        select3 = "건달패의 마차를 뒤져본다."; 

        result1 = "사람들이 연신 감사를 표한다. 평판이 좋아졌다."; //카리스마 +
        result2 = "높은 값에 팔 수 있었다. 많은 돈을 얻었다.";//돈 300
        result3 = "적국의 지도를 얻었다. 정보가 늘었다."; //정보력 2+ 
...
public:
    Block12()
    {
        script = "노상점을 만났다. 어서 옵쇼! \"1인 1물품 구매만 가능합니다. 신중히 골라보세요!\""; //스크립트 초기화

        select1 = "제목이 지워진 책을 산다.";
        select2 = "효능을 알 수 없는 포션을 산다."; 
        select3 = "딱히 살 물건이 없어보이니 그냥 간다."; 
...
public:
    Block15()
    {
        ...
        select1 = "몸이 좋아지게 해달라고 소원을 빈다.";
        select2 = "임무를 성공할 수 있게 해달라고 소원을 빈다."; 
        select3 = "연못에 있는 동전을 챙긴다.";
...
public:
    BlockChoiceStat()
    {
        ...
        select1 = "저는 뛰어난 학자입니다.";
        select2 = "저는 용맹한 군인입니다."; 
        select3 = "저는 돈많은 백수입니다.";
